 ...
 * @brief   openmp parallel fir low-pass filter (hamming window)
 * @usage   ./fir_openmp <input.bin> <n_samples> <num_taps> <cutoff_ratio> <num_threads> [-v]
 */
...
/**
 * @brief  apply fir filter in parallel using openmp; output samples are fully
 *         independent so no synchronization is needed between iterations
...
static void fir_parallel(const float *in, float *out, int n,
    ...
    // each output sample is independent — parallelize outer loop directly,
...
int main(int argc, char *argv[]) {
    ...
    printf("fir_openmp n=%d taps=%d threads=%d time_ms=%.4f\n",
