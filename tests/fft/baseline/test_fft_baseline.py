# test_fft_baseline.py
# author: Vaidehi Gohil, Anthony Yalong
# runs correctness tests for fft_serial against numpy.fft as reference
...
def run_fft_serial(bin_path, n):
    """
    Run fft_serial with -v and parse magnitude output from stdout.

    Returns numpy array of float magnitudes, one per frequency bin.
    """
    result = subprocess.run(
        [FFT_BIN, bin_path, str(n), "-v"],
        capture_output=True, text=True
    )
    assert result.returncode == 0, f"fft_serial failed:\n{result.stderr}"
