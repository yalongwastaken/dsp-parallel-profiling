# test_fft_pthreads.py
# author: Vaidehi Gohil, Anthony Yalong
# validates fft_pthreads output against fft_baseline across thread counts and input types
...
PTHREADS_BIN  = os.path.join(os.path.dirname(__file__), "../../../fft/pthreads/fft_pthreads")
...
def run_pthreads(bin_path, n, num_threads):
    """
    Run fft_pthreads with -v and return output magnitudes as a numpy array.

    First line of stdout is the timing summary, remaining lines are magnitudes.
    """
    result = subprocess.run(
        [PTHREADS_BIN, bin_path, str(n), str(num_threads), "-v"],
        capture_output=True, text=True
    )
    assert result.returncode == 0, f"fft_pthreads failed (threads={num_threads}):\n{result.stderr}"
    lines = result.stdout.splitlines()
    return np.array([float(l) for l in lines[1:] if l.strip()])
...
def test_multi_tone():
    # two sine waves — checks that independent frequency bins are unaffected
    # by thread boundaries
    n = 1024
    t = np.arange(n)
...
def main():
    ...
    print("=== fft_pthreads correctness tests ===")
    passed = 0
    for t in tests:
        try:
            t()
