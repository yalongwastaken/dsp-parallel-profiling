# test_fir_pthreads.py
# author: Vaidehi Gohil, Anthony Yalong
# validates fir_pthreads output against fir_baseline across thread counts and input types
...
PTHREADS_BIN  = os.path.join(os.path.dirname(__file__), "../../../fir/pthreads/fir_pthreads")
...
def run_pthreads(bin_path, n, num_threads, num_taps=NUM_TAPS, cutoff=CUTOFF):
    """
    Run fir_pthreads with -v and return output samples as a numpy array.

    First line of stdout is the timing summary, remaining lines are samples.
    """
    result = subprocess.run(
        [PTHREADS_BIN, bin_path, str(n), str(num_taps), str(cutoff), str(num_threads), "-v"],
        capture_output=True, text=True
    )
    assert result.returncode == 0, f"fir_pthreads failed (threads={num_threads}):\n{result.stderr}"
    lines = result.stdout.splitlines()
    return np.array([float(l) for l in lines[1:] if l.strip()], dtype=np.float32)
...
def run_against_baseline(label, samples, num_taps=NUM_TAPS, cutoff=CUTOFF):
    """
    Run baseline once, then compare pthreads output at each thread count.

    Helper used by all test cases to avoid duplicating the loop.
    """
    n = len(samples)
    ...
    try:
        ...
        for nt in THREAD_COUNTS:
            got = run_pthreads(path, n, nt, num_taps, cutoff)
...
def test_chunk_boundary():
    ...
    # catches errors where a thread reads samples owned by another thread's chunk
    n = 4096
...
def main():
    ...
    print("=== fir_pthreads correctness tests ===")
    passed = 0
    for t in tests:
        try:
            t()
